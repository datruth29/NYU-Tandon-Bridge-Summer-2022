...
__Solution:__  This function is NEITHER one-to-one NOR onto. The elements $\{1, 2\}$ and $\{2\}$ will both match with $\{2\}$ in the target, such that $f(\{1, 2\}) = f(\{2\}) = \{2\}$, therefore showing it can't be one-to-one.

No element in the $P(A)$ where $f(x) = \{1\}$, therefore showing it can't be onto.
