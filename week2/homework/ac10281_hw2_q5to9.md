...
Let $x$ be a real number and $x >= 3$. Since $x >= 3$, we can show that $x-3>=0$ 

Since $x$ is a real number, then $12-7x+x^2$ is a real number. And since $x-3>=0$, we can say that $12-7x+x^2>=x-3$
